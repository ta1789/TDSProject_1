...
def get_data_dir():
    """Determine the correct DATA_DIR for both local and Docker environments."""
    if os.path.exists("/.dockerenv"):  # Check if running inside Docker
        return "/data"  # Use Linux-style path inside the container
    else:
        return "C:/data" if os.name == "nt" else "/home/user/data"  # Default local paths

# Set DATA_DIR dynamically
...
def validate_file_path(file_path: str) -> bool:
    ...
    abs_allowed_dir = os.path.abspath(os.path.normpath(DATA_DIR))

    # Debugging logs
    print("Checking file path:", abs_file_path)
    print("Allowed directory:", abs_allowed_dir)
...
def install_uv_and_run_script(email: str):
    #venv_python = "C:/Users/ASUS/PycharmProjects/TDS_Project1/.venv/Scripts/python.exe"
...
def format_markdown_file(file_path: str,version: str):
    print(Path(file_path))
    subprocess.run(["npx", version, "--write", file_path], check=True,shell=True)
...
def count_days(file_path: str, output_file: str,day:str):
    ...
    file.close()
    with open(f'/data/date-{day}s.txt', "a") as f:
...
# Function to determine Tesseract path based on environment
def get_tesseract_path():
    if os.path.exists("/.dockerenv"):  # Running inside Docker
        return "tesseract"  # Use system-installed Tesseract in Docker
    else:  # Running on local machine (Windows)
...
def extract_credit_card_from_image(image_path: str, output_file: str):
    try:
        ...
        # Step 2: Prepare the prompt to pass the extracted text to LLM
        prompt = f"""
    ...
    try:
        ...
        if operation in ["B1", "B2", "B3", "B4", "B5", "B6", "B7", "B8", "B9", "B10"]:
            ...
            task_mapping = {
                ...
                "A2": lambda: format_markdown_file(file_path,params.get("prettier_version","prettier@3.4.2")),
                ...
                "A9": lambda: find_most_similar_comments(params.get("file_path", "/data/comments.txt"), params.get("output_file", "/data/comments-similar.txt")),
                "A10": lambda: calculate_gold_ticket_sales(params.get("db_path", "/data/ticket-sales.db"), params.get("output", "/data/ticket-sales-gold.txt")),
            }
